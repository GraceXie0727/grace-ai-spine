...
两个 Claude Code 的 Agent Skill，一个负责做（grace-builder），一个负责审查（grace-critic）。配对使用，让 AI 对你的文章、方案、想法做真正不顺着你的审查——不是"总体不错，建议优化一下"那种废话，而是逐条引用原文举证、打分、回答"该不该叫停"。

解决的问题：**AI 默认只会鼓掌。** 你让它审稿，它说"很好，可以考虑加一些……"；你让它提意见，它先夸三段再轻轻带一句。

这两个 skill 就是把 AI 的讨好模式关掉。
